...
class DashboardScreen: UIViewController, CLLocationManagerDelegate {
    ...
    func setupLabels(){
        ...
        let phone: String = defaultValues.string(forKey: "mobile")!
        ...
        Alamofire.request("http://128.199.178.5:8080/garbageback/garbageapi/rewards-status", method: .post, parameters: parameters, encoding: JSONEncoding.default).responseString{(response)->Void in
            response.result.ifSuccess {
                self.rewardLabel.text="Total Earnings\n\n\n"+response.result.value!+"MB"
                
            }
            response.result.ifFailure {
                self.rewardLabel.text="Error retrieving the information"
            }
            
        }
        
        typeLabel.text="Waste Type \n\n\nE-Waste"
